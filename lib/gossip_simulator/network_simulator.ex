defmodule GossipSimulator.NetworkSimulator do
  ...
  def top_3D(non) do
    ...
    Enum.map_every(ud+1..s-ud-ud+1,ud,fn x ->
      Enum.each(x..x+w-3,fn z->
        GenServer.cast(Enum.at(nodes,z), {:set_neighbours, [Enum.at(nodes, z-1),Enum.at(nodes, z+1), Enum.at(nodes, z-ud),Enum.at(nodes, z+ud)]})
        IO.puts("#{z}-#{z-1},#{z+1},#{z-w},#{z+w}")
      end)
    end)
    ...
    Enum.map_every(w+ud..ud-w-w+ud,w,fn x ->
      Enum.map_every(x..x+(ud*(w-3)),ud,fn z->
        GenServer.cast(Enum.at(nodes,z), {:set_neighbours, [Enum.at(nodes, z-w),Enum.at(nodes, z+w), Enum.at(nodes, z-ud),Enum.at(nodes, z+ud)]})
        IO.puts("#{z}-#{z-1},#{z+1},#{z-w},#{z+w}")
      end)
    end)
